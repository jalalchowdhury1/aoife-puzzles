...
| 34 | **Fill the Gap red-team text pass (2026-09-12, same day as #33)** | Owner: "check, recheck and redteam". Three free-model lanes plus Claude's own read of all 80 items. **Fixed:** fg-15 `chalky` was a defensible answer (chalky medicine also explains needing honey) scoring 0, now `yummy`; fg-37's example used *deliberate* as slow and careful while the card defined it as on purpose, new example uses the on-purpose sense; fg-78's definition no longer says every phenomenon is amazing. Because fg-15's OPTIONS changed after the 19:07Z release, the 80-item bank as released is frozen as legacy revision `REVISION_2026_09_12B` (`banks/legacy/2026-09-12b/`), cutover = this release moment; `cues.test.ts` pins both sides. Card-only text (definition, example) never needs a revision: replay validity reads prompt and options only. Code findings (practiceQueue drawOpts, the non-staircase generate branch, pickWidening reduce) were checked against the code and are false. |
