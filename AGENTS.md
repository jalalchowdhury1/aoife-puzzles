...
| 12 | Telegram | One summary message per completed part to the owner's @ZingerJC_bot DM — **OFF by default since 11 Sep 2026** (nag review: the health hub already confirms the event, scores live on the parent page). `PART_SUMMARY_PINGS=1` in Vercel turns it back on; the KV idempotency path is unchanged. |
...
| 30 | **Level 10 "Pip's Big Party": a level that cannot reach her wall (2026-09-05)** | Jalal: "prepare Aoife's next puzzle session. Make sure she loves it and doesn't have to press 'it's not fun'… Ends on a winning note." Level 9 (a ceiling probe by design) produced THREE Not-fun taps, one per part — arithmetic from d15 (miss in 5s, bail on item 2), fillTheGap at d10, whichTwo on its FIRST item at d5 (76s, the re-authored uncued bank) — and 5 of 6 blocks ended on a loss. Owner choices: win-heavy but still honest (#18 recipe); **no engine change** (a non-counting "victory lap" item after a two-miss stop was offered and declined — if ever wanted it is ~20 lines in staircase/runner); whichTwo restarts at d2 on the new bank. THE RULE: every block is structurally unable to serve the step that beat her — with stepUp 2 and no fast lane a flawless N-item block answers its last item at start + floor((N−1)/2), so 6-item blocks pinned at arithmetic 10 / information 8 / whichTwo 2 (+2 teaching items) / fillTheGap 7 / whatWouldYouDo 7 / swapShop 7 reach 12/10/4/9/9/9 against walls 15/12/5/10/10/10, and a normal run ends on `maxItems` (all dots filled, blockDone praise), not `twoWrong`. Parts = one bailed genre at a safe step → a confident closer (A Cake Sums 🎂 arithmetic→information; B Party Words 🎈 whichTwo→whatWouldYouDo; C Prize Table 🏆 swapShop→fillTheGap); both timed genres on 1.5×. Guards: `levels.test.ts` pins the WALL table (update only from her data, never to fit a level) and `level10.shape.test.ts` drives the real staircase to prove the clean-run shape. Spec `docs/superpowers/specs/2026-09-05-level10-big-party-design.md`. Parked: whatWouldYouDo topped d10 in L9C → a d11-15 ladder is EARNED under #17 (bank-authoring job, Level 11). |
| 31 | **Level 11 "Pip's Next Step": climb where flawless, hold where missed; Story Sums probes (2026-09-09)** | Level 10 came back 33/36 with ZERO Not-fun taps, every block to its last dot. Jalal: "go for it. so the math one you can test her limits. start higher. cause she's really good at math." Rule: a genre climbs only if its Level 10 block was flawless (arithmetic 10→13, whichTwo 2→3 with 4 items + 1 teaching, reach d4 < wall 5); every other start equals Level 10's with fresh seeds. **Story Sums is the one deliberate probe**: d13, eight items, reaching d16 — one past the d15 wall (which was a single 5s miss on item one of a block that STARTED at d15); easeIn gives d16 a free miss + 1.5× on top of the block's 1.5×. No other block may pass its wall; wall table unchanged. No engine/bank/scale change. Guards: `levels.test.ts` Level 11 describe + `level11.shape.test.ts`. Spec `docs/superpowers/specs/2026-09-09-level11-next-step-design.md`. |
| 32 | **Level 12 "Pip's Sky Picnic": step back where beaten, climb where flawless, nothing reaches a wall (2026-09-10)** | Level 11 came back 30/36, ZERO Not-fun taps, zero timeouts. The Story Sums probe answered its question: misses at d13 (29s), d14 (9s), d15 (3s guess) — her wall is d14-15, and it stays 15 in the table (no bail/timeout). Jalal: "Always remember it has to be win heavy and she MUST enjoy it." So Story Sums steps BACK (start 11, 6 items → d13, ends on a win), whatWouldYouDo climbs (flawless 6/6 → start 8, but 4 items so it reaches d9 < wall 10), whichTwo holds at 3 (flawless but d4 took 44s; start 4 would reach the d5 wall), information/swapShop/fillTheGap hold with fresh seeds. No block reaches a wall; no engine/bank/scale change. Built through /cos (gateway deepseek-v4-flash drafted, luna + flash red-teamed). Guards: `levels.test.ts` Level 12 describe + `level12.shape.test.ts`. Spec `docs/superpowers/specs/2026-09-10-level12-sky-picnic-design.md`. |
| 33 | **Fill the Gap variety pass + word card (2026-09-12)** | Jalal: "Many of the questions for fill the gap seems to be repetitive. Also when she has finished answering whether right or wrong …. Make sure to show her the definition of the word appropriate for a six year old with a small example sentence." Root cause from her data: only 5 items per tier, `excludeBankIds` resets every block, and her levels start her at d6-8, so the same words (fg-31, fg-33, fg-38 four times each) came back session after session. **Fixes:** (1) soft cross-session avoid list: `GET /api/state` gives fillTheGap blocks `avoidBankIds` from `servedBankIds()` (ids only, no scores); `pickWidening` prefers unserved items, else the least recently served at the SAME d, never widens; an empty list is the old behaviour exactly; the list is recorded on each item so history replay regenerates the same item. (2) 30 new items fg-51..fg-80, five per tier d5-d10, varied frames and parts of speech, all cue guards green. (3) the 50-item bank as it was is frozen as legacy revision `REVISION_2026_09_12` (set to the release moment). (4) `components/WordCard.tsx`: after EVERY Fill the Gap answer, right or not, on the play page and the Practice tab: the word, a kid definition (≤15 words, never contains the word) and an example (≤12 words, word in bold); read aloud once Pip stops talking; the advance timer parks until its "Got it!". (5) `PracticeRef.opts` now carries asOf + the block's earlier words + the avoid list, so a rematch is the same word she missed (before this, a rematch could silently become a different item); practice records now also store `bankId` + `drawOpts`, so the parent history view replays the word she actually saw or refuses. Guards: word-card coverage + avoid behaviour in `lib/genres/fillTheGap.test.ts`, `lib/engine/servedBankIds.test.ts`, practice opts tests, playthrough e2e taps the card. |
| 34 | **Fill the Gap red-team text pass (2026-09-12, same day as #33)** | Owner: "check, recheck and redteam". Three free-model lanes plus Claude's own read of all 80 items. **Fixed:** fg-15 `chalky` was a defensible answer (chalky medicine also explains needing honey) scoring 0, now `yummy`; fg-37's example used *deliberate* as slow and careful while the card defined it as on purpose, new example uses the on-purpose sense; fg-78's definition no longer says every phenomenon is amazing; fg-04's definition no longer says "a still place" (a six-year-old reads that as a location). Because fg-15's OPTIONS changed after the 19:07Z release, the 80-item bank as released is frozen as legacy revision `REVISION_2026_09_12B` (`banks/legacy/2026-09-12b/`), cutover = this release moment; `cues.test.ts` pins both sides. Card-only text (definition, example) never needs a revision: replay validity reads prompt and options only. Code findings (practiceQueue drawOpts, the non-staircase generate branch, pickWidening reduce) were checked against the code and are false. Tally: 52 model findings, 4 real (all word text), 48 false; most data false alarms were "the 1 point word also fits", which is the partial credit rule working. |
| 35 | **Level 13 "Pip's Treasure Map" + fresh questions in every bank genre (2026-09-14)** | Jalal: "we can't be repeating the same questions. Fresh ones will challenge her more. This is for her regular questions NOT the rematches." GET /api/state now gives EVERY bank genre its soft avoid list (was Fill the Gap only); rematches unchanged. Level 13 built from Level 12 (27/30). Her 37 sessions had zero unseen Information items at d8 to d10 and zero unseen Which Two items at d3 and d4, so in-61..72 and wt-76..82 were added (Claude written after Flash drafts failed), with ar-101..112 and wd-51..56. Old banks frozen under legacy/2026-09-14; REVISION_2026_09_14 = the release moment. Proof: a simulation of the real staircase and pickers against her served ids gave zero repeats for a flawless run and for any single miss in both blocks; headless self-play of parts A, B and C on prod (every /api call except GET /api/state stubbed, nothing written to KV) served Information 6/6 and Which Two 4/4 unseen, zero console errors. |
...
- 2026-09-09: **Level 11 "Pip's Next Step" shipped** (decision #31). Her Level 10 results (all three parts 2026-09-09): 33/36, zero Not-fun taps — arithmetic d10→12 6/6 (d12 in 0.5s), information d8→10 5/6 (miss d10, 35-40s), whichTwo d2→4 6/6 (no teaching item used), whatWouldYouDo d7→8 5/6 (miss d8), swapShop d7→9 5/6 (miss d9), fillTheGap d7→9 5/6 (miss d8). Level 11 = same win-heavy machinery; climbs only where flawless (arith 13 as an 8-item PROBE to d16 on owner order; whichTwo 3 with 4 items + 1 teaching); info/wwyd/swap/fillTheGap held at L10 starts with fresh seeds. Self-played all three parts headless (Playwright, throwaway spec, `next start` with KV env blanked, her 27 real sessions in localStorage): every block on its pinned start, teaching miss recorded, stepUp-2 climb seen, zero page errors. Position resolves to L11A on prod.
- 2026-09-05: **Level 10 "Pip's Big Party" shipped** (decision #30). Her Level 9 results: three Not-fun taps (arithmetic d15 item 2, fillTheGap d10, whichTwo d5 item 1 on the re-authored bank), swapShop 67s timeout at d10, information 7/8 median 8s (her happiest block), whatWouldYouDo 7/8 to d10 but 23-45s per item. Level 10 = six 6-item blocks hand-pinned below every wall (arith 10 / info 8 / whichTwo 2 + 2 teaching / fillTheGap 7 / whatWouldYouDo 7 / swapShop 7; 1.5× clock on the two timed genres; stepUp 2, fastLane off, easeIn on, reveal). No engine, bank, or scale change; her record untouched. Self-played all three parts headless (Playwright, throwaway profile, KV-less dev, her 23 real sessions injected into localStorage): every block started on its pinned step, whichTwo's first two misses recorded `teaching`, stepUp-2 climbs and no solo climbs, all three stickers, zero page errors. Position resolves to L10A on prod.
