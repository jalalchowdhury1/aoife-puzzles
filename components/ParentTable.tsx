 ...
 * everything here is relative to Aoife's own results.
 */
export function ParentTable({ profile }: { profile: Profile }) {
          ...
          <table className="w-full min-w-[760px] text-left text-sm text-ink">
            <thead>
              <tr className="border-b border-teal-100">
                ...
                <th className="py-2 font-semibold">Per minute</th>
              </tr>
            </thead>
            <tbody>
              {genreIds.length === 0 && (
              ...
              {genreIds.map((g) => {
                ...
                return (
                  <tr key={g} className="border-b border-teal-50">
                    ...
                    <td className="py-2 tabular-nums">{gs.perMinute !== undefined ? gs.perMinute.toFixed(1) : "—"}</td>
                  </tr>
                );
              })}
            </tbody>
