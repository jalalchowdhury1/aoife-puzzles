...
const BANK_CHOICE_GENRES = new Set<GenreId>([
  "fillTheGap", "whatWouldYouDo", "whichTwo", "information", "similarities", "vocabulary", "comprehension",
]);
...
export function computeInsights(sessions: SessionRecord[]): Insights {
  ...
  const genreOrder: GenreId[] = [...GENRE_LIST, ...RETIRED_GENRE_IDS.filter((g) => blocksByGenre.has(g))];
