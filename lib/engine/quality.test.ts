...
describe("flagBlock: format-not-understood", () => {
  it("flags when the first two non-teaching items are both wrong at d <= 2", () => {
    ...
    const flags = flagBlock(block("matrix", "staircase", items));
  ...
  it("ignores teaching items when finding the first two", () => {
    ...
    expect(flagBlock(block("matrix", "staircase", items))).toEqual([]);
  });
  ...
  it("does not flag wrong items above difficulty 2", () => {
    ...
    expect(flagBlock(block("matrix", "staircase", items)).map((f) => f.code)).not.toContain("format-not-understood");
  });
  ...
  it("flags a 2-item block with zero correct even when both items were teaching items (real incident: Piece Picker/What's Missing)", () => {
    ...
    const flags = flagBlock(block("visualPuzzles", "staircase", items));
    expect(flags.map((f) => f.code)).toContain("format-not-understood");
  });
  ...
  it("does not flag a normal block with a couple of misses spread through it", () => {
    ...
    expect(flagBlock(block("vocabulary", "staircase", items))).toEqual([]);
  });
});
...
describe("flagBlock: rapid-wrong", () => {
  it("flags when 2+ wrong, non-timed-out answers land under 2s", () => {
    ...
    const flags = flagBlock(block("digitSpan", "staircase", items));
  ...
  it("does not count a timed-out item even if its recorded ms is small", () => {
    ...
    expect(flagBlock(block("digitSpan", "staircase", items)).map((f) => f.code)).not.toContain("rapid-wrong");
  });
...
describe("flagBlock: mass-timeouts", () => {
  it("flags at >= 50% of attempted with attempted >= 3", () => {
    ...
    expect(flagBlock(block("blockDesign", "staircase", items)).map((f) => f.code)).toContain("mass-timeouts");
  });
  ...
  it("does not flag under the attempted >= 3 floor", () => {
    ...
    expect(flagBlock(block("blockDesign", "staircase", items)).map((f) => f.code)).not.toContain("mass-timeouts");
  });
});
...
describe("flagBlock: speed-accuracy", () => {
  it("flags under 70% accuracy with attempted >= 10", () => {
    ...
    expect(flagBlock(block("coding", "speedBlock", items)).map((f) => f.code)).toContain("speed-accuracy");
  });
  ...
  it("does not flag under the attempted >= 10 floor", () => {
    ...
    expect(flagBlock(block("coding", "speedBlock", items)).map((f) => f.code)).not.toContain("speed-accuracy");
  });
  ...
  it("does not apply to a staircase block even at low accuracy", () => {
    ...
    expect(flagBlock(block("matrix", "staircase", items)).map((f) => f.code)).not.toContain("speed-accuracy");
  });
});

describe("flagBlock: abandoned", () => {
  it("flags a block with zero attempts, and nothing else", () => {
    expect(flagBlock(block("matrix", "staircase", []))).toEqual([{ code: "abandoned", detail: "No items attempted." }]);
  });
});
...
describe("flagSession", () => {
  it("maps flagged block indices to their flags, skipping clean blocks", () => {
    ...
    const s: SessionRecord = {
      ...
      complete: true,
      appVersion: "0.1.0",
      blocks: [block("matrix", "staircase", clean), block("visualPuzzles", "staircase", bad)],
    };
...
describe("ensureFlags", () => {
  ...
  it("backfills flags on a block that lacks them", () => {
    const [filled] = ensureFlags([session("01FLAGBACKFILLTEST0000001", [block("visualPuzzles", "staircase", badItems)])]);
