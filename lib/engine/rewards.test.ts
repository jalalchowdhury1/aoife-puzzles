...
describe("sessionStars / totalStars", () => {
  it("sums explicit item.stars when present", () => {
    ...
    const s = session({ id: "s1", blocks: [block("matrix", "staircase", items)] });
  ...
  it("falls back to starsForItem when item.stars is undefined", () => {
    ...
    const s = session({ id: "s2", blocks: [block("matrix", "staircase", items)] });
  ...
  it("mixes explicit and fallback stars across blocks", () => {
    ...
    const s = session({
      id: "s3",
      blocks: [block("matrix", "staircase", withStars), block("vocabulary", "staircase", withoutStars)],
    });
    expect(sessionStars(s)).toBe(6);
  });

  it("totalStars sums sessionStars across sessions", () => {
    const s1 = session({ id: "s4", blocks: [block("matrix", "staircase", [item({ idx: 0, stars: 2 })])] });
    const s2 = session({ id: "s5", blocks: [block("matrix", "staircase", [item({ idx: 0, stars: 3 })])] });
...
describe("dayStreak", () => {
  function completeAt(id: string, startedAt: string): SessionRecord {
    return session({ id, startedAt, blocks: [block("matrix", "staircase", [item({ idx: 0 })])] });
  }
...
describe("newBests", () => {
  it("reports a genre whose ceiling beats the earlier best", () => {
    const earlier = [session({ id: "e1", blocks: [block("matrix", "staircase", [item({ idx: 0, d: 5 })], { summary: summarize([item({ idx: 0, d: 5 })], "staircase") })] })];
    const current = session({ id: "c1", blocks: [block("matrix", "staircase", [item({ idx: 0, d: 7 })])] });
    const result = newBests(current, earlier);
    expect(result).toEqual([{ genre: "matrix", ceiling: 7, previous: 5 }]);
  });

  it("returns previous: null for a genre never seen before", () => {
    const current = session({ id: "c2", blocks: [block("blockDesign", "staircase", [item({ idx: 0, d: 4 })])] });
    const result = newBests(current, []);
    expect(result).toEqual([{ genre: "blockDesign", ceiling: 4, previous: null }]);
  });

  it("does not report a genre that did not beat its earlier best", () => {
    const earlier = [session({ id: "e1", blocks: [block("matrix", "staircase", [item({ idx: 0, d: 8 })])] })];
    const current = session({ id: "c3", blocks: [block("matrix", "staircase", [item({ idx: 0, d: 6 })])] });
  ...
  it("ignores speedBlock genres (no ceiling)", () => {
    ...
    const current = session({ id: "c4", blocks: [block("coding", "speedBlock", items)] });
  ...
  it("takes the best ceiling within the session when a genre repeats (remedial repeat block)", () => {
    ...
    const current = session({
      ...
      blocks: [
        block("figureWeights", "staircase", [item({ idx: 0, d: 4 })], { startedAt: "2026-08-24T10:00:00.000Z", endedAt: "2026-08-24T10:01:00.000Z" }),
        block("figureWeights", "staircase", [item({ idx: 0, d: 6 })], { startedAt: "2026-08-24T10:02:00.000Z", endedAt: "2026-08-24T10:03:00.000Z" }),
      ],
    });
    expect(newBests(current, [])).toEqual([{ genre: "figureWeights", ceiling: 6, previous: null }]);
  });

  it("remaps ceilings onto the current scale before comparing (figureWeights rebuilt 2026-08-23)", () => {
    // Pre-cutover session recorded old-scale ceiling 4, which remaps to 6.
    const earlier = [session({ id: "e1", startedAt: "2026-08-20T10:00:00.000Z", blocks: [block("figureWeights", "staircase", [item({ idx: 0, d: 4 })])] })];
    // Post-cutover session at raw ceiling 6 (no remap applied, scale is current) does NOT beat the remapped previous best of 6.
    const current = session({ id: "c6", startedAt: "2026-08-24T10:00:00.000Z", blocks: [block("figureWeights", "staircase", [item({ idx: 0, d: 6 })], { startedAt: "2026-08-24T10:00:00.000Z", endedAt: "2026-08-24T10:01:00.000Z" })] });
    expect(newBests(current, earlier)).toEqual([]);
  });
});
