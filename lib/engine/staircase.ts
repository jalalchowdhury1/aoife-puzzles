...
export interface StairState { d: Difficulty; consecutiveWrong: number; consecutiveCorrect: number; items: number; ceiling: number | null; done: boolean; reason?: "twoWrong" | "maxItems" | "topReached"; maxItems: number; teachingItems: number; stepUp: number }
...
export function startStair(start: number | { fromProfileCeiling: number | null }, maxItems: number, teachingItems = 0, stepUp = 1): StairState {
  ...
  return { d, consecutiveWrong: 0, consecutiveCorrect: 0, items: 0, ceiling: null, done: false, maxItems, teachingItems, stepUp: Math.max(1, stepUp) };
}
export function stepStair(s: StairState, correct: boolean): StairState {
  ...
  if (correct) {
    ...
    const climb = consecutiveCorrect >= s.stepUp;
  ...
  if (consecutiveWrong >= 2) return { ...s, items, consecutiveWrong, consecutiveCorrect: 0, done: true, reason: "twoWrong" };
  const next = { ...s, items, consecutiveWrong, consecutiveCorrect: 0 };
  return items >= s.maxItems ? { ...next, done: true, reason: "maxItems" } : next;
}
