...
const RAMP: Partial<Record<GenreId, string>> = {
  ...
  patternTrain:
    "d1-4: colour/shape ABAB, AAB AAB, ABC ABC cycles. d5-6: count grows to 4 alone, then combined with a colour ABAB. " +
    "d7-8: a 4-cycle colour/shape, then a colour-3-cycle + shape-2-cycle combo. d9-10: count grows in pairs, then a " +
    "2-way colour/shape interleave. Extended past 10 (2026-08-24, earned — she solved a genuine level-10 item): " +
    "d11 introduces 'peak and mirror' — count climbs to 4 then mirrors back down one step at a time. d12 combines " +
    "d10's interleave with d11's mirror. d13 runs colour 3-cycle + shape 2-cycle + count growth all at once for the " +
    "first time. d14 is a genuine 3-way interleave (colour/shape/count each own every-3rd-car stream). d15 " +
    "(capstone, 5 options) is a 3-way interleave where one stream is itself a fully-shown peak-and-mirror, for extra " +
    "camouflage, while the answer continues the colour chain.",
  pictureSudoku:
...
<p class="subtitle">3 seeded items (seeds 1, 2, 3) per difficulty (d1&ndash;d10, wider for any genre that has earned it past 10 &mdash; see AGENTS.md decision #17), every genre. Generated by <code>scripts/audit-items.ts</code> &mdash; regenerate with <code>npx tsx scripts/audit-items.ts</code> before a release.</p>
